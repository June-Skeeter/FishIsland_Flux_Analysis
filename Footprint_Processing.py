...
class Calculate(object):
	...
	def __init__(self,out_dir,Data,Domain,XY,params=None,Classes=None,nx=1000,dx=1,rs=[.50,.75,.90],ax=None,OtherClass = 'Other',Name='',Write_Rasters = False):
	...
	def run(self):
		for i in range(self.Runs):
			...
			FP = FFP_climatology(zm=[self.Data['Zm'].iloc[i]],z0=[self.Data['Zo'].iloc[i]],h=[self.Data['hpbl'].iloc[i]],ol=[self.Data['L'].iloc[i]],
		...
		Contours(self.out_dir,Sum = self.Sum,raster_params=self.raster_params,ax=self.ax,r=self.rs,Name=self.Name)
...
class Contours(object):
	...
	def __init__(self,RasterPath,Sum=None,raster_params=None,ax=None,Jobs=None,r=[.25,.50,.70,.80,.90],PlotStyle=None,Name=''):
