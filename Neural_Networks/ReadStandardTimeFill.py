...
class ReadStandardTimeFill:#[1e-6 * 44.0095 *3600,1e-3 * 16.04246 *3600]
    def __init__(self,Data_Path,Save_Path='temp/',CombineKeys=[],Conversions=[1,1e3],resample=None,Const=False,FPVars=None):
        self.Save_Path=Save_Path
        self.Master = pd.read_csv(Data_Path,delimiter = ',',header = 0,na_values = -9999)
    ...
    def Scale(self,y_var,X_vars,ScalePath = None,Project=False,fillTarget=None):
        ...
        if ScalePath is None:
            ...
            joblib.dump(self.YScaled, self.Save_Path+scaler_filename) 
            scaler_filename = "X_scaler.save"
            joblib.dump(self.XScaled, self.Save_Path+scaler_filename)
